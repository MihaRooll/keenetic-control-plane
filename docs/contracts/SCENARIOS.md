...
| Check | Action |
|---|---|
| Phase | **P3 shared executor + topology safety complete** (2026-07-23; offline/default-deny); **next task:** `local-hub-vpn-real-peer-autoconnect-continuation` per [`STATUS.yaml`](../STATUS.yaml) `next_task` — VPN handshake + traffic via tunnel **device-verified** (§M-24..§M-27); station apply **live** (§M-34) |
| Phase 1 / offline mega | **Complete** (2026-07-21) — SLICE-2/3/5/8 delivered |
| Phase 1 / SLICE-4 Gate A | **Complete** (2026-07-21) — ReadOnlyCertified exact NC-1812 tuple |
| Gates A/B/C/D | **A open** (ReadOnlyCertified, live read-only only); **B** completed_failed; **C/D closed** — write dispatch → **403** |
| ID families | `SCN-EVT-*` event lifecycle; `SCN-ZONE-*` four zones; `SCN-OPS-*` operator happy path; `SCN-NEG-*` fail-closed negatives; `SCN-JOB-*` job failures/recovery; `SCN-LAB-*` future lab evidence; `SCN-CUT-*` strangler/cutover; `SCN-PR-*` portable rack / WISP client uplink (see [`SCENARIO_PORTABLE_EQUIPMENT_RACK.md`](../SCENARIO_PORTABLE_EQUIPMENT_RACK.md)) |
...
| Apply/verify | Happy path through **Confirm** is L1/L2-spec; **apply dispatch remains blocked** — Gate **B** **completed_failed** (not WriteCertified); Gates **C/D** **closed**; **L2 fake** full lifecycle is the SLICE-1 implementation target; successful verify/`applied_revision` via fake and/or future **L5–L7** hardware evidence only — never **L4** (Gate A read-only observe) |
...
**Current (2026-08-05):** **P3 shared executor + topology safety complete** (offline/default-deny; source_address bind; Gate D default-deny; registries empty). **P1-A/P1-B complete** (offline/fake; live MutationForbidden). **P2 immutable deployment complete** (2026-07-22). **M4/M5 complete** — recovery substrate + offline/default-deny framework. **`tunnel_healthy` DEVICE-CONFIRMED** (first real handshake §M-24..§M-26); **`SET_IP_ADDRESS` + `wireguard_ip_global` DEVICE-VERIFIED** (§M-24/M-27); traffic via tunnel **device-verified reversible** (§M-27). **Next task:** `local-hub-vpn-real-peer-autoconnect-continuation` per [`STATUS.yaml`](../STATUS.yaml) `next_task`. **Parallel deferred:** VPN named policy / kill-switch live apply (offline preview only; kill-switch `permit global` **unresolved**; **`CLEAR_IP_GLOBAL` on teardown** not device-proven; IPv6 allow-ips refused). Gate **A** open **ReadOnlyCertified**; Gate **B** **completed_failed**; Gates **C/D** **closed**; `write_shapes_registered` false; WriteCertified **NOT** claimed; fake-only resume/compensate; live mutations forbidden outside expendable envelope.
...
| **gates** | A/B/C/D status for this scenario — default **current posture** (A ReadOnlyCertified; B completed_failed; C/D closed) unless marked **historical Phase 0b** or negative closed-case |
...
|---|---|---|---|---|---|---|---|---|---|
| **SCN-EVT-001** | Pre-event setup | Operator (Admin/Server + `hub_admin`) | Hub up; `HUB_ADMIN_PASSWORD` configured; feature `Ready`; Gate **A** ReadOnlyCertified; `RC_ADAPTER_MODE=live` for live observe leg | **A** ReadOnlyCertified; **B** completed_failed; **C/D** Closed | `GET /status`; `POST /routers`; `POST .../preflight`; `PUT .../credentials`; `POST /vpn-profiles/import`; `POST .../validate`; `PUT .../desired-revision`; `POST .../plans`; `POST .../confirm` | Feature `Ready`; **L2 fake:** **202** + `PendingEnrollment` (SQLite intent, no live I/O). **Live path:** preflight observe succeeds → **`Enrolled`**; apply still **403** (B not WriteCertified). Plan `confirmation_state` `Confirmed`; no `applied_revision` advance | `enroll_requested`, `preflight_succeeded` (live) or `enroll_intent_persisted` (L2 fake), `credential_stored`, `profile_imported`, `desired_revision_created`, `plan_confirmed` | **L1** contract trace; **L2** fake; **L4** Gate A live observe when enabled | SECURITY §1–4; PERSISTENCE §3–4; HARDWARE §3 |
...
| **SCN-EVT-004** | Replacement / re-enrollment | Operator | Hardware replaced; old identity revoked; new enrollment ceremony; Gate **A** ReadOnlyCertified | **A** ReadOnlyCertified; **B** completed_failed; **C/D** Closed | `POST .../credentials/{id}/revoke`; `POST /routers` (new enrollment); `PUT .../credentials`; `POST .../preflight` (live observe when `RC_ADAPTER_MODE=live`) | Old refs revoked; new router **`PendingEnrollment`** (L2 fake) or **`Enrolled`** after live preflight; new VPN keys policy ([`SECURITY_OPS.md`](SECURITY_OPS.md) §7); no reuse of prior AWG material; apply still blocked (B not WriteCertified) | `credential_revoked`, `replacement_enroll_requested`, `preflight_succeeded` (live) | **L1** lifecycle spec; **L2** fake; **L4** Gate A live observe when enabled | SECURITY §7; DOMAIN_MODEL Router lifecycle |
...
| **SCN-OPS-002** | Enroll and identity | Operator | Valid `hub_admin`; enroll payload with write-only password; Gate **A** ReadOnlyCertified | **A** ReadOnlyCertified; **B** completed_failed; **C/D** Closed | `POST /routers`; `POST .../preflight` | **L2 fake:** **202** + SQLite intent, `PendingEnrollment`. **Live path:** preflight observe succeeds → **`Enrolled`** when `RC_ADAPTER_MODE=live`; apply still **403** (B not WriteCertified) | `enroll_intent_persisted` (L2) or `preflight_succeeded` (live) | **L1** + **L2** fake; **L4** Gate A live observe when enabled | API §10.1; HARDWARE §3 |
| **SCN-OPS-003** | Credentials write-only | Operator | Valid `hub_admin`; router enrollment intent (`PendingEnrollment` or `Enrolled`); vault-only — preflight/`Enrolled` not required for credential store | **A** ReadOnlyCertified; **B** completed_failed; **C/D** Closed | `PUT .../credentials`; `GET .../credentials`; `GET .../credentials/{id}` | **201** ref; GET returns metadata only (`kind`, timestamps) — **no** plaintext | `credential_stored` | **L1** + **L2** redaction | SECURITY §4; API §7.5 |
...
| **SCN-OPS-009** | Apply through verify (write-blocked) | Operator | Confirmed unexpired plan | **B** completed_failed; **C/D** Closed | `POST .../plans/{plan_id}/apply` | **Current:** **403** `gate.mutation_forbidden` (B not WriteCertified). **Future (L5–L7):** operation `Applying`→`Verifying`→`Converged`; `applied_revision_id` only after read-back verify. **L2 fake** may simulate full apply/verify path (never **L4** — Gate A is read-only observe only) | `apply_denied_gate` or future `apply_succeeded`, `verify_complete` | **L1** fail-closed note; **L2** fake full lifecycle; **L5–L7** future hardware write evidence | API §10.2; RCI §5; PERSISTENCE §3.2 |
...
### 3.6a Portable rack / WISP uplink (`SCN-PR-*`)

Normative detail and gap register: [`SCENARIO_PORTABLE_EQUIPMENT_RACK.md`](../SCENARIO_PORTABLE_EQUIPMENT_RACK.md). **Offline foundation + bounded live association (2026-07-31 CLI)** — HTTP station apply/teardown delivered offline 2026-08-01 (NOT device-verified live).

| id | title | actor | preconditions | gates | scope | Phase 0b / current | trace |
|---|---|---|---|---|---|---|---|
| **SCN-PR-001** | Venue WifiWan uplink preset | Field technician (via operator preset) | Event preset with `UplinkIntent.mode=WifiWan`; CredentialRef for venue PSK | **B** not WriteCertified; WifiWan preset planner **unsupported** / `wifi_wan_not_certified` | Parse/validate/plan preview; HTTP `POST .../wifi/station/apply` + `POST .../wifi/station/teardown` **delivered offline** (confirm-gated; **NOT device-verified live**) | **Offline spec** + sealed HTTP routes; preset `wifi_wan_not_certified`; no WriteCertified | SCENARIO_PORTABLE; ROADMAP §3.2 |
| **SCN-PR-002** | Captive portal client uplink | Field technician | `captive_portal_client=true` on uplink | Same as PR-001 | Readiness `uplink_captive_portal_client_unsupported`; distinct from host Coova-Chilli | **Not supported** | SCENARIO_PORTABLE §4 |
...
| SCN-PR-001 | wifi station apply/teardown POST (offline delivered; **NOT device-verified live**) | L1, L2 | SCENARIO_PORTABLE, API §wifi-station |
| SCN-PR-002 | uplink captive-portal readiness (unsupported) | L1 | SCENARIO_PORTABLE §4 |
...
**Current state:** Phase 1 offline mega **complete** and SLICE-4 Gate A read-only **exit complete** (2026-07-21); **M0–M5 complete** (2026-07-22); **P1-A/P1-B complete** (offline/fake; live MutationForbidden); **P2 immutable deployment complete** (2026-07-22); **P3 shared executor + topology safety complete** (2026-07-23; registries empty) per [`STATUS.yaml`](../STATUS.yaml) — Gate **A** open ReadOnlyCertified exact NC-1812 tuple (post-WG rebind 2026-07-31); live read-only enroll/preflight/commissioning assess when `RC_ADAPTER_MODE=live`; Gate **B** completed_failed (not WriteCertified); Gates **C/D** closed; `write_shapes_registered` false; **`tunnel_healthy` DEVICE-CONFIRMED** (first real handshake §M-24..§M-26); **`SET_IP_ADDRESS` + `wireguard_ip_global` DEVICE-VERIFIED** (§M-24/M-27); traffic via tunnel **device-verified reversible** (§M-27). **Next task:** `local-hub-vpn-real-peer-autoconnect-continuation` per [`STATUS.yaml`](../STATUS.yaml) `next_task`. **Parallel deferred:** VPN named policy / kill-switch live apply (offline preview only; kill-switch `permit global` **unresolved**; **`CLEAR_IP_GLOBAL` on teardown** not device-proven; IPv6 allow-ips refused). Dedicated HW validation on project-owned NC-1812 is **in scope** under program authorization; every live mutation requires a separate T4 Human Gate Packet when outside expendable envelope. Prototype UI covers SCN-OPS read/poll/cancel boundaries without Apply when gates closed. AWG shape discovery / Gate B re-open remains a **parallel deferred lane**.
