...
| POST | `/keendns/observe` | Read-only live KeenDNS/CrazeDNS observe (`show acme` + `show ndns`; optional `get-booked`) | Sync | — | — | Wifi-style live connection fields; live path: Gate A required → `open_wifi_live_session` RO only (no backup/writes/confirm); fake adapter when no live params; response `default_fqdn`, `ssl_valid`, `booked_fqdn` / `booked_name`+`booked_domain`, `access_mode`, `name_reservation`, `notes`, **`certification_eligible: false`**; empty/missing default-domain → null (never invent FQDN); errors `keendns.*` |
...
| POST | `/keendns/apply` | Dispatch sealed `ndns book-name` / `drop-name` on expendable lab | Sync | — | — | **`confirm_live_apply: true` required** (else **400** `keendns.confirm_required`); wifi-style live connection fields; live path: Gate A open → expendable fail-closed → live tuple probe (mismatch → **422** `keendns.identity_mismatch`, zero backup/writes) → backup → allowlisted writes; errors `keendns.*` (`component_absent`, `expendable_required`, `apply_failed`, …); **not WriteCertified**; cloud registration not verified by host |
...
| POST `/keendns/observe` | `KeenDnsObserveResponse` | Live RO `show acme` + `show ndns`; `default_fqdn` + `ssl_valid` from sealed parse only; **`certification_eligible: false`** |
...
### 13.8 Additive — live mutation identity tuple mismatch (2026-08-08)

**Impact:** Live apply/teardown routes probe the connected device tuple immediately after `open_wifi_live_session` and **before** startup-config backup or mutation. Mismatch or incomplete probe evidence → **422** `{family}.identity_mismatch` with **zero backup and zero writes**. Distinct from **503** `{family}.gate_a_required` (cert closed/absent) and **503** `{family}.live_backup_unavailable` (backup failure after tuple match).

| Route | HTTP | `error.code` |
|---|---|---|
| POST `/wifi/apply`, `/wifi/teardown` | **422** | `wifi.identity_mismatch` |
| POST `/wifi/station/apply`, `/wifi/station/teardown` | **422** | `wifi.station.identity_mismatch` |
| POST `/wireguard/apply`, `/wireguard/teardown` | **422** | `wireguard.identity_mismatch` |
| POST `/keendns/apply` | **422** | `keendns.identity_mismatch` |

**Unchanged:** read-only observe/survey paths (`POST /wifi/observed-state`, `/wifi/site-survey`, WireGuard observe, …) — tuple probe helper is **not** inside `open_wifi_live_session`.

**Evidence:** `ensure_live_gate_a_tuple_match()` in `router_control_host/wifi_live_transport.py`; tests `test_wifi_live_wiring.py`, `test_wifi_station_apply_api.py`, `test_wireguard_live_wiring.py`, `test_keendns_live_wiring.py`.
