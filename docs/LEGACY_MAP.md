...
| `Monitor-CursorNetwork.ps1` SSH/plink/plain environment credential paths | **Do-not-reuse** | Password в environment/command invocation и optional `accept-new` host key не соответствуют target vault/trust boundaries. Target: explicit learn→confirm→pin via `POST …/ssh-host-key/learn|confirm` ([`API_CONTRACT.md`](contracts/API_CONTRACT.md) §7.9); blind TOFU forbidden ([`SECURITY_OPS.md`](contracts/SECURITY_OPS.md)). | `CredentialVault`, authenticated RCI transport, `ssh_host_key_pin` application service |
