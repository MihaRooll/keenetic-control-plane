...
The current deployment observation on the lab NC-1812 is raw firmware **`5.01.C.1.0-0`** (display **`5.1.1`**) with NDM build **`0-b592e619a0`**, BSP **`0-f371d30955`**, channel **Main**, region **EA**, and a post-change Gate A ReadOnlyCertified component-set digest (2026-07-21 re-probe; **return-home source-bound recertification** refreshed Gate A evidence on **2026-07-23** with same tuple and `source_address: 192.168.1.144`). Gate A does not certify individual component presence; Gate B preflight must freshly re-observe it. The firmware value must **not** be normalized to **`5.1`** alone: official pages use forms such as `5.1`, `5.1.0`, and `5.1.1`, while the Help Center index and latest Main release page are not fully consistent.
...
**Milestone context:** Local-first commissioning order is [ADR-0005](adrs/0005-local-first-commissioning-roadmap.md) / [`ROADMAP.md`](contracts/ROADMAP.md). **P3 topology safety closure complete** (2026-07-23): shared Certified registry wiring, Gate D default-deny, explicit `source_address` bind for overlapping `192.168.1.0/24` labs — **next task:** `local-hub-vpn-real-peer-autoconnect-continuation` per [`STATUS.yaml`](STATUS.yaml) `next_task` (VPN handshake + traffic via tunnel **device-verified** §M-24..§M-27; station apply **live** §M-34) — **parallel deferred:** VPN named policy / kill-switch live apply (offline preview only; kill-switch `permit global` **unresolved**; **`SET_IP_ADDRESS` + `wireguard_ip_global` DEVICE-VERIFIED** §M-24/M-27; IPv6 allow-ips refused; Gate B / `write_shapes_registered` BLOCKED (not WriteCertified)). On overlapping dual-homed lab topology, explicit `source_address` bind (`192.168.1.144`) is required for live observe; **future live mutations** additionally require WAN isolation or physical uplink disconnect — source binding alone does not authorize writes. **M5 offline/default-deny framework delivered** (2026-07-22). **M4 complete** (2026-07-22): durable recovery substrate offline/fake (**not live-ready**). AWG write lane (Gate B trial **completed_failed** 2026-07-21) is a **parallel deferred** family certification — not a predecessor of M1–M4 offline work; `write_shapes_registered: false` unchanged.
...
| WireGuard | **Verified:** NC-1812 documentation supports the WireGuard component and NDMS 5.0 server UI. The post-change Gate A evidence records only the installed-component-set digest, not individual component presence. | **Unsupported in v1** | Detect/report only; Router Control v1 accepts AmneziaWG profiles only. Gate B preflight must freshly re-observe exact component presence; no Gate A component-presence claim authorizes writes. |
