# New-chat orchestrator prompt — 2026-08-07 ВЕЧЕР (редизайн главного экрана: 4 карточки отшлифованы параллельными агентами; НЕ завершены финальные шаги)

## For agents

| Факт | Значение |
|---|---|
| Назначение | Живой блок для вставки — запуск нового Main-оркестратора Router Control |
...
| Главное отличие | Главный экран «Обзор» доведён до карточной сетки по референсу оператора: фундамент починен Main лично (§M-60), четыре карточки отшлифованы четырьмя ПАРАЛЛЕЛЬНЫМИ Grok-оркестраторами, Main лично нашёл и починил 4 дефекта после их возврата. **Финальные шаги НЕ сделаны — см. §1.** |
| Do not | Заявлять, что редизайн «сдан»: полный прогон тестов прерван, `CACHE_VERSION` не поднят, живая пиксельная приёмка 4 карточек не выполнена, `STATUS.yaml`/decisions за этот раунд не обновлены |
...
## 1. ПЕРВЫМ ДЕЛОМ — незакрытые шаги предыдущей сессии (в этом порядке)

Предыдущий Main остановился ровно здесь. Ничего не переделывай заново — только доведи:
