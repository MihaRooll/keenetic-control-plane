"""NC-1812 default-route discovery probe — non-certifying GET /rci/show/ip/route."""
...
def _resolve_source_address(raw: str, *, required: bool) -> tuple[int, str | None]:
    if not raw.strip():
        if required:
            print("--source-address is required for live default-route probe", file=sys.stderr)
            return 2, None
        return 0, None
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="NC-1812 default-route discovery probe (non-certifying, source-bound)."
    )
    ...
    parser.add_argument(
        "--artifact-out",
        default="",
        help="Sanitized default-route artifact JSON path",
    )
    parser.add_argument(
        "--shape-out",
        default="",
        help="Optional structural fingerprint JSON path on RouteTopologyProbeError",
    )
    parser.add_argument(
        "--topology-artifact",
        default="",
        help="Optional sanitized topology v2.2 artifact for hash correlation",
    )
    parser.add_argument(
        "--gate-a-config",
...
def _load_gate_a_aligned(
    *,
    ...
    if not gate_a.is_open:
        raise GateACertificationError("Gate A is not open for aligned default-route probe")
...
def _fixture_digests(*, fixture_name: str) -> tuple[str, str]:
    ...
    return (
        ...
        digest_evidence_record({"fixture": fixture_name}),
    )
...
def _fixture_shape_artifact(
    *,
    ...
    tuple_digest, evidence_digest = _fixture_digests(fixture_name=fixture_name)
    return build_default_route_shape_artifact(
        payload=payload,
        raw_bytes=raw_bytes,
...
def _run_fixture(
    *,
    ...
    artifact_out: Path,
    source_address: str,
    topology_artifact_path: Path | None,
) -> dict[str, object]:
    from router_control.adapters.netcraze.route_topology_probe import (
    ...
    validated_source = validate_source_address(source_address or "192.168.2.10")
    tuple_digest, evidence_digest = _fixture_digests(fixture_name=fixture_name)
    artifact = build_default_route_artifact(
        payload=payload,
        raw_bytes=raw_bytes,
        source_address=validated_source,
        source_address_class=source_address_class(validated_source),
    ...
    if topology_artifact_path is not None:
        topology_payload = json.loads(topology_artifact_path.read_text(encoding="utf-8"))
        if not isinstance(topology_payload, dict):
            raise ValueError("topology artifact must be an object")
        correlation = correlate_with_topology_artifact(artifact, topology_payload)
        artifact = dict(artifact)
        artifact["topology_correlation"] = {
            "status": correlation.status.value,
            "default_outbound_hashes": list(correlation.default_outbound_hashes),
            "connected_non_lan_hashes": list(correlation.connected_non_lan_hashes),
            "overlapping_hashes": list(correlation.overlapping_hashes),
            "topology_classification": correlation.topology_classification,
            "notes": list(correlation.notes),
        }
...
def main() -> int:
    ...
    topology_path = Path(args.topology_artifact.strip()) if args.topology_artifact.strip() else None
    ...
    if fixture_mode:
        ...
        from router_control.adapters.netcraze.route_topology_probe import RouteTopologyProbeError
        ...
        try:
            _run_fixture(
                ...
                topology_artifact_path=topology_path,
            )
        except RouteTopologyProbeError as exc:
    ...
    if missing:
        print("Live default-route probe requires: " + ", ".join(missing), file=sys.stderr)
        return 2
    ...
    payload: object = {}
    raw_bytes = b""
    transport: SshTunnelNetcrazeTransport | None = None
    ...
    try:
        with PinnedSshTunnel(tunnel_config) as tunnel:
            ...
            payload = transport.fetch_discovery_read(SHOW_IP_ROUTE)
        ...
        shape_path = args.shape_out.strip()
        if shape_path and transport is not None:
            shape_artifact = build_default_route_shape_artifact(
                payload=payload,
                raw_bytes=raw_bytes,
                parser_error=exc,
                source_address=validated_source,
        ...
        print(f"Default-route probe failed: {exc.__class__.__name__}", file=sys.stderr)
        return 4
