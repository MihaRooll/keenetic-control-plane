...
export function describeVpnAutoReconnectNote({
  ...
  if (hasActiveAssignment === false) {
    return 'Процесс автопереподключения на сервере управления может быть включён, но активного назначения профиля нет — переподключение не восстановит туннель, пока профиль не подключён или не назначен.';
  }
  return 'Автоматическое переподключение включено в сервере управления, но назначение активного профиля ещё не загружено — при сбое VPN неизвестно, восстановится ли туннель без ручного подтверждения; работа на роутере не подтверждена.';
}
