"""DNS Configure → Apply service (offline sealed executor; not composition-wired).

Preview/build is deterministic and safe offline. ``apply_dns_intent`` requires an
...
until Gate B DNS certification / WriteCertified.
"""
...
from router_control.application.dns_apply_planner import (
    ...
    derive_dns_pre_state,
    uncovered_compensate_ops_for_succeeded_dns_apply,
)
...
_MSG_LIVE_DISPATCH_DISABLED = (
    "live DNS apply dispatch is disabled; inject transport for offline tests only"
)
_MSG_OP_DISPATCH_FAILED = ERROR_CODE_OP_DISPATCH_FAILED


class DnsApplyServiceError(ValueError):
    """Fail-closed VLAN apply service error."""


class DnsApplyTransport(Protocol):
    dns_offline_only: Literal[True]

    def execute_sealed_rci_write(self, request: SealedRciWriteRequest) -> Any: ...


def require_dns_offline_transport(transport: object) -> None:
    """Fail-closed guard: block live NetcrazeHttpTransport unless explicitly marked offline-only."""
    if getattr(transport, "dns_offline_only", False) is not True:
        raise DnsApplyServiceError(_MSG_LIVE_DISPATCH_DISABLED)
...
@dataclass(frozen=True, slots=True)
class DnsApplyUncoveredRollbackOp:
    op: str
    reason: str
...
@dataclass(frozen=True, slots=True)
class DnsApplyRollback:
    attempted: bool
    ops: tuple[str, ...]
    outcome: ApplyRollbackOutcome
    steps: tuple[DnsApplyStep, ...] = ()
    uncovered_ops: tuple[DnsApplyUncoveredRollbackOp, ...] = ()
...
class DnsApplyResult:
    ...
    logs: tuple[str, ...]
    notes: tuple[str, ...] = ()
    rollback: DnsApplyRollback | None = None
...
def _op_to_preview_dict(op: DnsSealedOpDescriptor) -> dict[str, object]:
    payload: dict[str, object] = {"operation": op.operation, "zone_id": op.zone_id}
    if op.local_fqdn is not None:
        payload["local_fqdn"] = op.local_fqdn
    if op.upstream_resolver is not None:
        payload["upstream_resolver"] = op.upstream_resolver
    if op.notes:
        payload["notes"] = list(op.notes)
    return payload


def plan_to_preview_dict(plan: DnsApplyPlan) -> dict[str, object]:
    return {
        "zone_id": plan.zone_id,
        "local_fqdn": plan.local_fqdn,
        "upstream_resolvers": list(plan.upstream_resolvers),
...
def preview_dns_apply(intent: Mapping[str, Any]) -> dict[str, object]:
    """Validate + compile only; no dispatch."""
    plan = _compile_plan(intent)
    return plan_to_preview_dict(plan)


def _compile_plan(intent: Mapping[str, Any]) -> DnsApplyPlan:
    try:
        return compile_dns_intent_to_ops(intent)
    except (DnsApplyPlannerError, ValueError) as exc:
        raise DnsApplyServiceError(str(exc)) from exc


def _status_ident(result: DnsRciResult) -> str | None:
    if not result.status_entries:
        return None
    return result.status_entries[0].ident


def _step_from_result(op_name: str, result: DnsRciResult) -> DnsApplyStep:
    return DnsApplyStep(op=op_name, ok=True, status_ident=_status_ident(result))


def _step_from_error(op_name: str, message: str) -> DnsApplyStep:
    return DnsApplyStep(op=op_name, ok=False, error=message)


def _rollback_not_attempted() -> DnsApplyRollback:
    return DnsApplyRollback(attempted=False, ops=(), outcome="not_attempted")


def _rollback_noop() -> DnsApplyRollback:
    return DnsApplyRollback(attempted=True, ops=(), outcome="noop")


def _finalize_dns_rollback_outcome(
    *,
    rollback_steps: tuple[DnsApplyStep, ...],
    rollback_errors: tuple[str, ...],
    uncovered_ops: tuple[DnsApplyUncoveredRollbackOp, ...],
...
def _finalize_overall_with_rollback(
    ...
    *,
    rollback: DnsApplyRollback | None,
) -> ApplyOverallStatus:
...
def _dispatch_ops(
    *,
    transport: DnsApplyTransport,
    ops: tuple[DnsSealedOpDescriptor, ...],
    logs: list[str],
    continue_on_error: bool = False,
    trail: SealedApplyTrailHandle | None = None,
) -> tuple[tuple[DnsApplyStep, ...], tuple[str, ...]]:
    require_dns_offline_transport(transport)
    steps: list[DnsApplyStep] = []
    errors: list[str] = []
    for descriptor in ops:
        op_name = descriptor.operation
        try:
            operation = DnsRciOperation(op_name)
        except ValueError:
        ...
        try:
            result = execute_dns_rci(
                transport,
                operation,
                descriptor.zone_id,
                local_fqdn=descriptor.local_fqdn,
                upstream_resolver=descriptor.upstream_resolver,
            )
        except DnsRciError:
...
def _attempt_compensating_rollback(
    *,
...
) -> tuple[DnsApplyRollback, tuple[str, ...]]:
    uncovered_pairs = uncovered_compensate_ops_for_succeeded_dns_apply(
        apply_ops, succeeded_op_names, pre_state=pre_state
    )
    uncovered_ops = tuple(
        DnsApplyUncoveredRollbackOp(op=op_name, reason=reason)
        for op_name, reason in uncovered_pairs
    )
    for item in uncovered_ops:
        logs.append(f"compensate uncovered {item.op}: {item.reason}")
    compensate_ops = compensate_ops_for_succeeded_dns_apply(
        apply_ops, succeeded_op_names, pre_state=pre_state
    )
    ...
    outcome = _finalize_dns_rollback_outcome(
...
def apply_dns_intent(
    *,
    intent: Mapping[str, Any],
    transport: DnsApplyTransport | None = None,
    ...
    plan = _compile_plan(intent)
    baseline = pre_state if pre_state is not None else derive_dns_pre_state(None)
    ...
    def _run() -> DnsApplyResult:
        ...
        def _finish(result: DnsApplyResult) -> DnsApplyResult:
            finish_sealed_apply_trail(
                trail,
