...
**Phase 0a (architecture evidence) завершена.** **Phase 0b (contracts) — complete** (Wave 7 closeout). **Phase 1 / SLICE-1 (portable core + FakeRouterAdapter) — complete** (2026-07-21). **Phase 1 / SLICE-2 (persistence / jobs) — pending** — требует отдельного human approval перед кодом. Все eight contract deliverables согласованы ([`docs/contracts/`](docs/contracts/)). Supporting: hardware safety gates ([`docs/contracts/HARDWARE_GATES.md`](docs/contracts/HARDWARE_GATES.md)) — **gates A–D остаются closed**.

Python 3.11 package `router_control` создан (`pyproject.toml`, `router_control/`, fake-only `tests/`). FastAPI dev-host, SQLite persistence и live adapter **ещё не созданы**.
