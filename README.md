...
**Phase 0a (architecture evidence) завершена.** **Phase 0b (contracts) — complete** (Wave 7 closeout). **Phase 1 / SLICE-1 (portable core + FakeRouterAdapter) — in_progress** — explicit human approval 2026-07-21; `implementation_transition_gate.code_may_start=true` **только для SLICE-1**. Все eight contract deliverables согласованы: `rci-policy`, `security-ops`, `persistence-contract`, `api-contract`, `test-strategy`, `scenarios`, `roadmap`, `ai-handoff` ([`docs/contracts/`](docs/contracts/), [`docs/contracts/ROADMAP.md`](docs/contracts/ROADMAP.md), [`docs/contracts/AI_HANDOFF.md`](docs/contracts/AI_HANDOFF.md)). Supporting: hardware safety gates ([`docs/contracts/HARDWARE_GATES.md`](docs/contracts/HARDWARE_GATES.md)) — **gates A–D остаются closed**.

Кода приложения, Python package, FastAPI host и `pyproject.toml` в репозитории пока нет. SLICE-1 code **разрешён**; package **ещё не создан** — следующий task реализует portable core + FakeRouterAdapter + fake-only tests.
